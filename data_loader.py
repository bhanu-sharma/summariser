...
def batch(data, batch_size):
    ...
    for ex in data:
        ...
        print(f"size so far: {size_so_far}, batch_size: {batch_size}")
...
class DataIterator(object):
    ...
    def batch_buffer(self, data, batch_size):
        ...
        for ex in data:
            ...
            minibatch.append(ex)
            size_so_far = simple_batch_size_fn(ex, len(minibatch))
            # print(f"size so far: {size_so_far}, batch size: {batch_size}")
            if size_so_far == batch_size:
                yield minibatch
                minibatch, size_so_far = [], 0
            elif size_so_far > batch_size:
                # print(f"Behold! The MiniBatch {minibatch} \n and it's tail {len(minibatch)}")
    ...
    def create_batches(self):
        """ Create batches """
        data = self.data()
        # print(f"Loading train dataset from data/bert_data.train.pt, number of examples: {len(data)}")
        for buffer in self.batch_buffer(data, self.batch_size * 50):
            # print(f"The buffer we receive: {buffer} \n and it's tail {len(buffer)}")
