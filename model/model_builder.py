...
class Bert(nn.Module):
    ...
    def forward(self, x, segs, mask):
        ...
        print(f"encoded layers: {len(encoded_layers)}")
...
class Summarizer(nn.Module):
    ...
    def forward(self, x, segs, clss, mask, mask_cls, sentence_range=None):
        # print(f"len of x: {x.shape}, segs: {segs.shape}, mask: {mask.shape}")
        top_vec = self.bert(x, segs, mask)
        # print(f"len of top_vec {len(top_vec)}")
        sents_vec = top_vec[torch.arange(top_vec.size(0)).unsqueeze(1), clss]
        # print(f"len of sents_vec: {len(sents_vec)}, len of: {mask_cls[:, :, None]}")
        sents_vec = sents_vec * mask_cls[:, :, None].float()
        # print("all okay in this town folks")
